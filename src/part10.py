...
<div class="figcap"><b>登录后你拿到的「身份」就是这枚签名的 JWT</b>（形状取自 <code>next-auth.d.ts</code> 与 <code>auth.ts</code>，<code>session.strategy=&quot;jwt&quot;</code>；<b>值为示例</b>）：三段 <code>header.payload.signature</code>。<code>payload</code> 里除了 <code>sub</code>/<code>iat</code>/<code>exp</code>，还带着整个 <code>user</code>——尤其是 <code>organizations[].role</code> 和其下 <code>projects[].role</code>，这正是第 49 课 RBAC 判权的依据。<code>signature</code> 段保证完整性：改一个字节就对不上，无法伪造越权。<b>注意</b>：上图是<b>通用 JWT</b> 结构示意；Langfuse 未注册自定义 <code>jwt.encode/decode</code>，故 NextAuth v4 默认签发<b>加密的 JWE</b> session token——payload 实际<b>并非公开可读</b>，完整性由 AEAD 保证。会话时效由 <code>AUTH_SESSION_MAX_AGE</code> 控制。</div>
</div>
...
<div class="figcap"><b>Login entry</b>: <code>auth.ts:24-38</code> imports 14 providers (Credentials/Email + 12 SSO), plus <code>CustomSSOProvider</code>/<code>GitHubEnterpriseProvider</code>/<code>JumpCloudProvider</code> imported server-side, so ~15 SSO in all. Static providers come from env; dynamic SSO providers are read from the DB and merged at runtime (around <code>auth.ts:739</code>, <code>[...staticProviders, ...dynamicSsoProviders]</code>). SSO can be enforced: <code>AUTH_DISABLE_USERNAME_PASSWORD</code> (instance), <code>getSSOBlockedDomains()</code> (domain), <code>getSsoAuthProviderIdForDomain</code> (EE multi-tenant).</div>
</div>
<div class="fig">
...
<div class="figcap"><b>The "identity" you get after login is this signed JWT</b> (shape from <code>next-auth.d.ts</code> and <code>auth.ts</code>, <code>session.strategy=&quot;jwt&quot;</code>; <b>values illustrative</b>): three parts <code>header.payload.signature</code>. Beyond <code>sub</code>/<code>iat</code>/<code>exp</code>, the <code>payload</code> carries the whole <code>user</code> — notably <code>organizations[].role</code> and the nested <code>projects[].role</code>, which is exactly what Lesson 49's RBAC checks. The <code>signature</code> guarantees integrity: flip one byte and it no longer matches, so privilege can't be forged. <b>Note</b>: the panel above shows <b>generic JWT</b> structure; Langfuse registers no custom <code>jwt.encode/decode</code>, so NextAuth v4 issues an <b>encrypted JWE</b> session token by default — the payload is <b>not actually publicly readable</b>, with integrity via AEAD. Session lifetime is controlled by <code>AUTH_SESSION_MAX_AGE</code>.</div>
</div>
...
<p>先厘清一个常见误解：open-core <strong>不是「一半开源一半闭源」</strong>。Langfuse 这个仓库里<strong>所有功能的代码都是开放的</strong>——包括那些「企业级」功能。区别只在<strong>运行时能不能启用</strong>：每个组织挂着一个 <code>plan</code>，免费的默认是 <code>"oss"</code>（自托管开源版），付费的有云上五档 <code>cloud:hobby / core / pro / team / enterprise</code>，以及自托管两档 <code>self-hosted:pro / enterprise</code>。<code>isCloudPlan</code> / <code>isSelfHostedPlan</code> 这两个小助手按前缀区分它们。功能用不用得了，全看你的 plan 在不在那张允许名单里。</p>

<div class="fig">
<svg viewBox="0 0 720 240" role="img" aria-label="开源核模型：整个仓库代码全开放(核心+企业级功能都能读)，但运行时按组织的plan解锁——oss免费(自托管开源)、cloud五档(hobby/core/pro/team/enterprise)、self-hosted两档(pro/enterprise)；plan决定能启用哪些entitlement">
  ...
  <rect x="406" y="116" width="135" height="32" rx="6" fill="var(--blue-soft)" stroke="var(--blue)"/><text x="473" y="129" text-anchor="middle" font-size="6.2" fill="var(--ink)">cloud:pro/team/enterprise</text><text x="473" y="140" text-anchor="middle" font-size="6.5" fill="var(--muted)">云·高级</text>
...
<div class="layers">
  ...
  <div class="layer l-main"><div class="lh"><span class="badge">付费</span><span class="name">cloud / self-hosted 各档</span></div><div class="ld">云上 hobby→core→pro→team→enterprise、自托管 pro→enterprise，逐级解锁更多企业功能、放宽更多额度。同一套代码，按 plan 给不同组织开不同的「门」。</div></div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>plan 分层</strong>：组织挂一个 <code>plan</code>，默认 <code>"oss"</code>（免费自托管），付费有 cloud:hobby/core/pro/team/enterprise + self-hosted:pro/enterprise；<code>isCloudPlan</code>/<code>isSelfHostedPlan</code> 按前缀分。</li>
...
<p>First clear up a common misconception: open-core is <strong>not "half open-source, half closed"</strong>. In this Langfuse repo <strong>all features' code is open</strong> — including the "enterprise" ones. The difference is only <strong>whether they can be enabled at runtime</strong>: each org carries a <code>plan</code>, free defaults to <code>"oss"</code> (self-hosted open-source), paid has five cloud tiers <code>cloud:hobby / core / pro / team / enterprise</code> plus two self-hosted <code>self-hosted:pro / enterprise</code>. The helpers <code>isCloudPlan</code> / <code>isSelfHostedPlan</code> distinguish them by prefix. Whether a feature works hinges entirely on whether your plan is on its allow list.</p>

<div class="fig">
<svg viewBox="0 0 720 240" role="img" aria-label="Open-core model: the entire repo code is open (core + enterprise features both readable), but unlocked at runtime by the org's plan — oss free (self-hosted open-source), cloud tiers (hobby/core/pro/team/enterprise), self-hosted tiers (pro/enterprise); the plan decides which entitlements can be enabled">
  ...
  <rect x="406" y="116" width="135" height="32" rx="6" fill="var(--blue-soft)" stroke="var(--blue)"/><text x="473" y="129" text-anchor="middle" font-size="6.2" fill="var(--ink)">cloud:pro/team/enterprise</text><text x="473" y="140" text-anchor="middle" font-size="5.6" fill="var(--muted)">cloud · premium</text>
  <rect x="552" y="116" width="130" height="32" rx="6" fill="var(--purple-soft)" stroke="var(--accent)"/><text x="617" y="129" text-anchor="middle" font-size="6.0" fill="var(--accent-ink)">self-hosted:pro</text><text x="617" y="140" text-anchor="middle" font-size="6.0" fill="var(--accent-ink)">/enterprise</text>
...
<div class="layers">
  ...
  <div class="layer l-main"><div class="lh"><span class="badge">paid</span><span class="name">cloud / self-hosted tiers</span></div><div class="ld">Cloud hobby→core→pro→team→enterprise, self-hosted pro→enterprise, progressively unlocking more enterprise features and loosening more quotas. Same code, different "doors" opened for different orgs by plan.</div></div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>Plan tiering</strong>: an org carries a <code>plan</code>, defaulting to <code>"oss"</code> (free self-host), paid has cloud:hobby/core/pro/team/enterprise + self-hosted:pro/enterprise; <code>isCloudPlan</code>/<code>isSelfHostedPlan</code> split by prefix.</li>
