...
<svg viewBox="0 0 720 235" role="img" aria-label="数据集模型：一个 Dataset 含多个 DatasetItem，每个 item 有 input/expectedOutput/metadata；item 可以现编，也可以从一条真实生产 trace 经 sourceTraceId 提拔而来，把线上难题变成回归测试用例">
  ...
  <line x1="580" y1="186" x2="580" y2="168" stroke="var(--accent)" stroke-width="1.5"/><polygon points="580,166 576,175 584,175" fill="var(--accent)"/><text x="638" y="180" text-anchor="middle" font-size="6.5" fill="var(--accent-ink)">提拔</text>
...
<svg viewBox="0 0 720 240" role="img" aria-label="dataset run 模型：一个数据集的多道题在一次 run 中各被执行一次，每次执行产生一条 trace；DatasetRunItem 把 datasetRunId 加 datasetItemId 连到 traceId，形成 run×item→trace 的三元链接；多次 run 用不同配置考同一套题">
  ...
  <rect x="560" y="74" width="140" height="40" rx="8" fill="var(--teal)" opacity="0.16" stroke="var(--teal)"/><text x="630" y="91" text-anchor="middle" font-size="7.5" font-weight="700" fill="var(--teal)">trace A/B/C</text><text x="630" y="105" text-anchor="middle" font-size="6.5" fill="var(--muted)">每题一条真实 trace</text>
  <rect x="560" y="128" width="140" height="36" rx="8" fill="var(--bg)" stroke="var(--blue)" stroke-dasharray="3 2"/><text x="630" y="145" text-anchor="middle" font-size="7.5" font-weight="700" fill="var(--ink)">run「claude 场」…</text><text x="630" y="158" text-anchor="middle" font-size="6.5" fill="var(--muted)">同套题换配置再考</text>
  <line x1="140" y1="120" x2="288" y2="120" stroke="var(--accent)" stroke-width="1.4"/><polygon points="288,120 279,116 279,124" fill="var(--accent)"/><text x="214" y="113" text-anchor="middle" font-size="6.5" fill="var(--accent-ink)">执行每道题</text>
...
<svg viewBox="0 0 720 235" role="img" aria-label="服务端 prompt 实验：对数据集每道题，replaceVariablesInPrompt 把 item.input 填进 prompt 的变量占位符，用选定 provider/model 调 LLM，产出一条 trace（环境标 PromptExperiments、链接 prompt、钉住 item 版本）和一个 run item">
  ...
  <rect x="30" y="44" width="150" height="56" rx="9" fill="var(--purple-soft)" stroke="var(--accent)"/><text x="105" y="64" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">prompt（带变量）</text><text x="105" y="80" text-anchor="middle" font-size="6.8" fill="var(--accent-ink)">"回答 {{question}}"</text><text x="105" y="93" text-anchor="middle" font-size="6.5" fill="var(--muted)">第37课的 prompt</text>
  ...
  <rect x="225" y="80" width="150" height="56" rx="9" fill="var(--bg)" stroke="var(--teal)"/><text x="300" y="100" text-anchor="middle" font-size="8" font-weight="700" fill="var(--teal)">replaceVariablesInPrompt</text><text x="300" y="116" text-anchor="middle" font-size="6.6" fill="var(--muted)">填空 → 完整 messages</text><text x="300" y="128" text-anchor="middle" font-size="6.5" fill="var(--faint)">"回答 退款政策?"</text>
  <rect x="420" y="80" width="130" height="56" rx="9" fill="var(--accent-soft)" stroke="var(--accent)" stroke-width="2"/><text x="485" y="100" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">调 LLM</text><text x="485" y="116" text-anchor="middle" font-size="6.6" fill="var(--accent-ink)">provider/model</text><text x="485" y="128" text-anchor="middle" font-size="6.5" fill="var(--muted)">被考的配置</text>
  ...
  <rect x="588" y="112" width="112" height="44" rx="8" fill="var(--bg)" stroke="var(--blue)"/><text x="644" y="130" text-anchor="middle" font-size="7.5" font-weight="700" fill="var(--ink)">run item</text><text x="644" y="145" text-anchor="middle" font-size="6.5" fill="var(--muted)">钉住 item 版本</text>
