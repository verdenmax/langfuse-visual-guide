...
<svg viewBox="0 0 720 215" role="img" aria-label="read-through 缓存流程：应用请求 production 的 prompt，PromptService 先查 Redis，命中则秒返回；未命中则查 Postgres、把结果写进 Redis（带 TTL）再返回，下次同请求即命中">
  ...
  <rect x="220" y="80" width="150" height="60" rx="9" fill="var(--purple-soft)" stroke="var(--accent)" stroke-width="2"/><text x="295" y="100" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">PromptService</text><text x="295" y="117" text-anchor="middle" font-size="6.6" fill="var(--accent-ink)">① 先查 Redis</text><text x="295" y="129" text-anchor="middle" font-size="6.4" fill="var(--muted)">命中→返回</text>
  <rect x="430" y="40" width="150" height="48" rx="9" fill="var(--accent-soft)" stroke="var(--accent)"/><text x="505" y="60" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">Redis 缓存</text><text x="505" y="76" text-anchor="middle" font-size="6.4" fill="var(--muted)">毫秒级 · 带 TTL</text>
  <rect x="430" y="130" width="150" height="48" rx="9" fill="var(--teal)" opacity="0.16" stroke="var(--teal)"/><text x="505" y="150" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--teal)">Postgres（权威）</text><text x="505" y="166" text-anchor="middle" font-size="6.4" fill="var(--muted)">未命中才查</text>
  <line x1="160" y1="110" x2="218" y2="110" stroke="var(--blue)" stroke-width="1.5"/><polygon points="218,110 209,106 209,114" fill="var(--blue)"/>
  <line x1="370" y1="95" x2="428" y2="70" stroke="var(--accent)" stroke-width="1.5"/><polygon points="428,70 419,69 421,77" fill="var(--accent)"/><text x="399" y="78" text-anchor="middle" font-size="6" fill="var(--accent-ink)">① 命中</text>
  <line x1="370" y1="120" x2="428" y2="150" stroke="var(--teal)" stroke-width="1.3" stroke-dasharray="3 2"/><polygon points="428,150 419,148 421,156" fill="var(--teal)"/><text x="396" y="142" text-anchor="middle" font-size="6" fill="var(--teal)">② 未命中→查库</text>
  <path d="M 505 130 Q 560 110 580 90" fill="none" stroke="var(--accent)" stroke-width="1.3" stroke-dasharray="4 3"/><polygon points="580,90 572,95 578,98" fill="var(--accent)"/><text x="600" y="112" text-anchor="middle" font-size="6" fill="var(--accent-ink)">③ 回填 Redis</text>
...
<svg viewBox="0 0 720 195" role="img" aria-label="prompt 依赖解析：父 prompt 引用两个子 prompt，buildAndResolvePromptGraph 沿依赖图把子 prompt 内联进父，得到展开后的完整 prompt；过程有嵌套深度上限和 seen 环检测两道闸；缓存与返回的都是这个 resolved 成品">
  ...
  <rect x="30" y="48" width="150" height="100" rx="9" fill="var(--purple-soft)" stroke="var(--accent)"/><text x="105" y="68" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">父 prompt</text><rect x="44" y="78" width="122" height="24" rx="5" fill="var(--bg)" stroke="var(--faint)"/><text x="105" y="94" text-anchor="middle" font-size="6.4" fill="var(--muted)">引用 子A（label）</text><rect x="44" y="108" width="122" height="24" rx="5" fill="var(--bg)" stroke="var(--faint)"/><text x="105" y="124" text-anchor="middle" font-size="6.4" fill="var(--muted)">引用 子B（version）</text>
  <rect x="270" y="60" width="160" height="76" rx="9" fill="var(--bg)" stroke="var(--teal)" stroke-width="2"/><text x="350" y="80" text-anchor="middle" font-size="8" font-weight="700" fill="var(--teal)">buildAndResolvePromptGraph</text><text x="350" y="98" text-anchor="middle" font-size="6.6" fill="var(--muted)">沿图找子 prompt 内联</text><text x="350" y="113" text-anchor="middle" font-size="6.4" fill="var(--accent-ink)">闸1 深度上限</text><text x="350" y="126" text-anchor="middle" font-size="6.4" fill="var(--accent-ink)">闸2 seen 环检测</text>
  <rect x="520" y="60" width="170" height="76" rx="10" fill="var(--accent-soft)" stroke="var(--accent)" stroke-width="2"/><text x="605" y="84" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">resolved prompt</text><text x="605" y="102" text-anchor="middle" font-size="6.6" fill="var(--accent-ink)">子片段已全部内联</text><text x="605" y="116" text-anchor="middle" font-size="6.4" fill="var(--muted)">完整、自包含</text><text x="605" y="129" text-anchor="middle" font-size="6.2" fill="var(--faint)">← 缓存/返回的就是它</text>
...
<svg viewBox="0 0 720 210" role="img" aria-label="epoch 失效：缓存 key 形如 prompt:项目:epoch:名字:版本；prompt 变更时 invalidateCache 把项目级 epoch 令牌从 e1 转成 e2，所有带 e1 的旧 key 不再被读到、按 TTL 自然过期，新读写都落在 e2 命名空间">
  ...
  <rect x="60" y="92" width="270" height="80" rx="10" fill="var(--bg)" stroke="var(--faint)" stroke-dasharray="4 3"/><text x="195" y="112" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--muted)">旧 epoch = e1（失联）</text><text x="195" y="132" text-anchor="middle" font-size="6.6" fill="var(--muted)">prompt:proj:e1:greeting:production</text><text x="195" y="148" text-anchor="middle" font-size="6.6" fill="var(--muted)">prompt:proj:e1:refund:production</text><text x="195" y="164" text-anchor="middle" font-size="6.4" fill="var(--faint)">没人再读 → 按 TTL 自然过期</text>
  <rect x="390" y="92" width="270" height="80" rx="10" fill="var(--accent-soft)" stroke="var(--accent)" stroke-width="2"/><text x="525" y="112" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">新 epoch = e2（现役）</text><text x="525" y="132" text-anchor="middle" font-size="6.6" fill="var(--accent-ink)">prompt:proj:e2:greeting:production</text><text x="525" y="148" text-anchor="middle" font-size="6.6" fill="var(--accent-ink)">…新读写都落这里（先 miss 再回填）</text><text x="525" y="164" text-anchor="middle" font-size="6.4" fill="var(--muted)">相当于一整片干净缓存</text>
