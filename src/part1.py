...
<svg viewBox="0 0 720 220" role="img" aria-label="一条宽 observation 行的解剖：同一行里塞进三类字段——关联字段(id/traceId/parentObservationId 拼调用树)、数字字段(latency/usageDetails/costDetails 可聚合)、业务字段(model/userId/metadata/tags 可过滤分组)，于是按模型/用户/时间提问都是同一张宽表的列查询，无需JOIN">
  ...
  <rect x="24" y="80" width="216" height="118" rx="9" fill="var(--blue-soft)" stroke="var(--blue)"/><text x="132" y="100" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--ink)">① 关联字段（拼调用树）</text><text x="132" y="120" text-anchor="middle" font-size="7" fill="var(--muted)">id · traceId · projectId</text><text x="132" y="135" text-anchor="middle" font-size="7" fill="var(--muted)">parentObservationId</text><text x="132" y="150" text-anchor="middle" font-size="7" fill="var(--muted)">type · startTime/endTime</text><text x="132" y="172" text-anchor="middle" font-size="6.4" fill="var(--faint)">让它既是「链路节点」</text><text x="132" y="185" text-anchor="middle" font-size="6.4" fill="var(--faint)">又能独立查询</text>
  <rect x="252" y="80" width="216" height="118" rx="9" fill="var(--teal)" opacity="0.16" stroke="var(--teal)"/><text x="360" y="100" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--teal)">② 数字字段（可聚合）</text><text x="360" y="120" text-anchor="middle" font-size="7" fill="var(--muted)">latency · timeToFirstToken</text><text x="360" y="135" text-anchor="middle" font-size="7" fill="var(--muted)">usageDetails（token 用量）</text><text x="360" y="150" text-anchor="middle" font-size="7" fill="var(--muted)">costDetails · totalCost</text><text x="360" y="172" text-anchor="middle" font-size="6.4" fill="var(--faint)">直接 sum/avg/p95，</text><text x="360" y="185" text-anchor="middle" font-size="6.4" fill="var(--faint)">不必从日志里 parse</text>
  <rect x="480" y="80" width="216" height="118" rx="9" fill="var(--purple-soft)" stroke="var(--accent)"/><text x="588" y="100" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">③ 业务字段（可过滤分组）</text><text x="588" y="120" text-anchor="middle" font-size="7" fill="var(--muted)">model · internalModelId</text><text x="588" y="135" text-anchor="middle" font-size="7" fill="var(--muted)">metadata · environment</text><text x="588" y="150" text-anchor="middle" font-size="7" fill="var(--muted)">promptName/Version</text><text x="588" y="172" text-anchor="middle" font-size="6.4" fill="var(--faint)">「按模型 / 按环境」</text><text x="588" y="185" text-anchor="middle" font-size="6.4" fill="var(--faint)">退化成列谓词</text>
...
<svg viewBox="0 0 720 190" role="img" aria-label="三支柱落到三张分开的ClickHouse表：trace(列表主入口,量居中)、observation(量最大字段最宽查询最频繁)、score(量少常按关联查)各一张表，因访问模式不同而分表，各用最适合的排序键与分区，互不拖累；score 通过 traceId/observationId 关联，observation 通过 traceId 关联">
  ...
  <rect x="40" y="44" width="190" height="120" rx="9" fill="var(--blue-soft)" stroke="var(--blue)"/><text x="135" y="64" text-anchor="middle" font-size="9" font-weight="700" fill="var(--ink)">traces 表</text><text x="135" y="84" text-anchor="middle" font-size="6.6" fill="var(--muted)">列表页主入口</text><text x="135" y="98" text-anchor="middle" font-size="6.6" fill="var(--muted)">量居中</text><text x="135" y="118" text-anchor="middle" font-size="6.2" fill="var(--faint)">排序键/分区</text><text x="135" y="130" text-anchor="middle" font-size="6.2" fill="var(--faint)">为「按时间列表」优化</text>
  <rect x="265" y="44" width="190" height="120" rx="9" fill="var(--accent-soft)" stroke="var(--accent)" stroke-width="2"/><text x="360" y="64" text-anchor="middle" font-size="9" font-weight="700" fill="var(--accent-ink)">observations 表</text><text x="360" y="84" text-anchor="middle" font-size="6.6" fill="var(--accent-ink)">量最大·字段最宽</text><text x="360" y="98" text-anchor="middle" font-size="6.6" fill="var(--muted)">查询最频繁</text><text x="360" y="118" text-anchor="middle" font-size="6.2" fill="var(--faint)">宽事件主场</text><text x="360" y="130" text-anchor="middle" font-size="6.2" fill="var(--faint)">为聚合/过滤优化</text>
  <rect x="490" y="44" width="190" height="120" rx="9" fill="var(--purple-soft)" stroke="var(--accent)"/><text x="585" y="64" text-anchor="middle" font-size="9" font-weight="700" fill="var(--accent-ink)">scores 表</text><text x="585" y="84" text-anchor="middle" font-size="6.6" fill="var(--muted)">量最少</text><text x="585" y="98" text-anchor="middle" font-size="6.6" fill="var(--muted)">常按关联查</text><text x="585" y="118" text-anchor="middle" font-size="6.2" fill="var(--faint)">为「按 trace/obs 取分」优化</text>
...
<svg viewBox="0 0 720 200" role="img" aria-label="同一个 observation 的多次事件乱序到达：observation-create(开始,模型gpt-4o)与observation-update(输出X,1240token)分别到达，worker 回S3取这个id的历史事件，按id分组合并、冲突以更晚timestamp为准，收敛成一条最终ClickHouse行">
  ...
  <rect x="24" y="44" width="180" height="40" rx="8" fill="var(--blue-soft)" stroke="var(--blue)"/><text x="114" y="60" text-anchor="middle" font-size="7.6" font-weight="700" fill="var(--ink)">事件1 observation-create</text><text x="114" y="74" text-anchor="middle" font-size="6.2" fill="var(--muted)">{id:obs-42, 开始, model:gpt-4o}</text>
  ...
  <text x="114" y="156" text-anchor="middle" font-size="6.4" fill="var(--faint)">两条分别到达、可能乱序</text>
  <rect x="266" y="62" width="180" height="74" rx="9" fill="var(--accent-soft)" stroke="var(--accent)" stroke-width="2"/><text x="356" y="82" text-anchor="middle" font-size="8" font-weight="700" fill="var(--accent-ink)">worker 合并</text><text x="356" y="99" text-anchor="middle" font-size="6.4" fill="var(--accent-ink)">回 S3 取 id=obs-42 历史</text><text x="356" y="113" text-anchor="middle" font-size="6.4" fill="var(--muted)">按 id 分组、字段取有值方</text><text x="356" y="127" text-anchor="middle" font-size="6.4" fill="var(--muted)">冲突以更晚 timestamp 为准</text>
  <rect x="508" y="68" width="188" height="62" rx="9" fill="var(--teal)" opacity="0.16" stroke="var(--teal)"/><text x="602" y="88" text-anchor="middle" font-size="8" font-weight="700" fill="var(--teal)">一条最终行</text><text x="602" y="105" text-anchor="middle" font-size="6.4" fill="var(--muted)">obs-42：gpt-4o + output:X</text><text x="602" y="119" text-anchor="middle" font-size="6.4" fill="var(--muted)">+ 1240 token，写进 ClickHouse</text>
