...
<div class="cols">
  ...
  <div class="col">
    ...
    <p>一<strong>步</strong>具体操作，分三型：<code>GENERATION</code>（一次 LLM 调用，带 model/token/cost）、
    <code>SPAN</code>（一段有耗时的逻辑，如检索）、<code>EVENT</code>（一个瞬时事件）。它带着
    input/output、时间、用量、成本等<strong>丰富属性</strong>，是查询与聚合的主体。</p>
  </div>
</div>
...
<div class="card detail">
  ...
  observation 的三种类型在领域层是枚举（历史上叫 <code>LegacyPrismaObservationType</code>：<code>GENERATION / SPAN / EVENT</code>，见
  <code>packages/shared/prisma/schema.prisma</code>）。ClickHouse 的 <code>observations</code> 表把 model、
...
<div class="cols">
  ...
  <div class="col">
    ...
    <p>One concrete <strong>step</strong>, in three types: <code>GENERATION</code> (an LLM call, with
    model/token/cost), <code>SPAN</code> (a timed unit of logic, e.g. retrieval), <code>EVENT</code> (an
    instantaneous event). It carries <strong>rich attributes</strong> — input/output, timing, usage, cost —
    and is the subject of querying and aggregation.</p>
  </div>
</div>
...
<div class="card detail">
  ...
  The three observation types are an enum in the domain layer (historically <code>LegacyPrismaObservationType</code>:
  <code>GENERATION / SPAN / EVENT</code>, see <code>packages/shared/prisma/schema.prisma</code>). The ClickHouse
