...
<svg viewBox="0 0 720 240" role="img" aria-label="JobExecution 状态机：PENDING 创建后进入执行；成功则 COMPLETED 并记录产出的 scoreId；非可重试错误或重试预算耗尽则 ERROR；被后续事件取消选中则 CANCELLED；遇限流则 DELAYED 约 120 分钟后重试回到执行">
  ...
  <rect x="420" y="34" width="130" height="44" rx="9" fill="var(--teal)" opacity="0.18" stroke="var(--teal)" stroke-width="2"/><text x="485" y="53" text-anchor="middle" font-size="9" font-weight="700" fill="var(--teal)">COMPLETED ✓</text><text x="485" y="68" text-anchor="middle" font-size="6.5" fill="var(--muted)">记 jobOutputScoreId</text>
  <rect x="420" y="92" width="130" height="40" rx="9" fill="var(--bg)" stroke="var(--faint)"/><text x="485" y="110" text-anchor="middle" font-size="9" font-weight="700" fill="var(--ink)">DELAYED</text><text x="485" y="124" text-anchor="middle" font-size="6.5" fill="var(--muted)">限流，~120min 后重试</text>
  <rect x="420" y="146" width="130" height="40" rx="9" fill="var(--bg)" stroke="var(--accent)"/><text x="485" y="164" text-anchor="middle" font-size="9" font-weight="700" fill="var(--accent-ink)">ERROR ✗</text><text x="485" y="178" text-anchor="middle" font-size="6.5" fill="var(--muted)">不可重试/预算耗尽</text>
  <rect x="220" y="170" width="120" height="40" rx="9" fill="var(--bg)" stroke="var(--faint)" stroke-dasharray="4 3"/><text x="280" y="188" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--muted)">CANCELLED</text><text x="280" y="201" text-anchor="middle" font-size="6.5" fill="var(--muted)">被后续事件取消选中</text>
...
<svg viewBox="0 0 720 220" role="img" aria-label="代码 eval 的策略模式：worker 调用统一的 CodeEvalDispatcher 接口的 dispatch 方法，底层由 resolveConfiguredCodeEvalDispatcher 按环境变量选择 AwsLambda 派发器(按语言调 python/node Lambda 沙箱)或本地 vm 派发器(名为 insecure-local，仅 TS/JS)">
  ...
  <rect x="40" y="150" width="300" height="56" rx="10" fill="var(--blue-soft)" stroke="var(--blue)"/><text x="190" y="169" text-anchor="middle" font-size="9" font-weight="700" fill="var(--ink)">AwsLambdaCodeEvalDispatcher</text><text x="190" y="185" text-anchor="middle" font-size="6.8" fill="var(--muted)">name="aws-lambda" · 按语言调 Lambda 沙箱</text><text x="190" y="198" text-anchor="middle" font-size="6.5" fill="var(--faint)">python→executor-python · TS→executor-node（生产）</text>
  <rect x="380" y="150" width="300" height="56" rx="10" fill="var(--bg)" stroke="var(--accent)" stroke-dasharray="5 3"/><text x="530" y="169" text-anchor="middle" font-size="9" font-weight="700" fill="var(--accent-ink)">LocalCodeEvalDispatcher</text><text x="530" y="185" text-anchor="middle" font-size="6.8" fill="var(--muted)">name="insecure-local" · node vm 跑</text><text x="530" y="198" text-anchor="middle" font-size="6.5" fill="var(--faint)">仅 TS/JS · 名字直说「不安全」，仅本地开发</text>
...
<svg viewBox="0 0 720 235" role="img" aria-label="监控流水线：调度器每到 nextRunAt 就把到期的 monitor 入队；处理器用与仪表盘相同的 view/filters/metric 在最近 windowMs 窗口算出一个指标值；computeSeverity 按 operator 比 alert/warning 阈值得出严重度；只有严重度变化才发告警到 WebhookQueue 再转投递">
  ...
  <line x1="630" y1="98" x2="450" y2="148" stroke="var(--accent)" stroke-width="1.4" stroke-dasharray="4 3"/><polygon points="450,148 459,146 456,139" fill="var(--accent)"/><text x="556" y="128" text-anchor="middle" font-size="6.5" fill="var(--accent-ink)">仅当 emit=true</text>
...
<svg viewBox="0 0 720 215" role="img" aria-label="严重度状态机：指标值为空时按 noData 模式处理为 NO_DATA、补零或沿用上次；非空时若满足告警阈值则 ALERT、满足警告阈值则 WARNING、否则 OK；另有用户手动的 PAUSED 与初始 UNKNOWN">
  ...
  <rect x="40" y="104" width="150" height="44" rx="9" fill="var(--bg)" stroke="var(--ink)" stroke-dasharray="4 3"/><text x="115" y="122" text-anchor="middle" font-size="8" font-weight="700" fill="var(--ink)">NO_DATA</text><text x="115" y="137" text-anchor="middle" font-size="6.5" fill="var(--muted)">按 noData 模式：补零/沿用/标无数据</text>
  ...
  <text x="610" y="78" font-size="6.5" fill="var(--muted)">用户暂停</text><text x="600" y="128" font-size="6.5" fill="var(--muted)">初始/未知</text>
  <line x1="115" y1="84" x2="115" y2="102" stroke="var(--faint)" stroke-width="1.2"/><text x="150" y="98" font-size="6.5" fill="var(--muted)">空</text>
  <line x1="190" y1="58" x2="268" y2="58" stroke="var(--accent)" stroke-width="1.4"/><polygon points="268,58 259,54 259,62" fill="var(--accent)"/><text x="229" y="52" text-anchor="middle" font-size="6.5" fill="var(--accent-ink)">撞告警线</text>
  <line x1="190" y1="70" x2="268" y2="106" stroke="var(--accent)" stroke-width="1.2"/><polygon points="268,106 259,102 258,110" fill="var(--accent)"/><text x="226" y="92" text-anchor="middle" font-size="6.5" fill="var(--muted)">撞警告线</text>
  <line x1="190" y1="78" x2="268" y2="156" stroke="var(--teal)" stroke-width="1.2"/><polygon points="268,156 259,153 258,161" fill="var(--teal)"/><text x="220" y="140" text-anchor="middle" font-size="6.5" fill="var(--teal)">都没撞</text>
...
<svg viewBox="0 0 720 200" role="img" aria-label="状态机决定是否发告警：上次 OK 这次 ALERT 则发；上次 ALERT 这次仍 ALERT 默认不发、除非 renotify 周期到；恢复为 OK 也发一条；告警统一发到 WebhookQueue，再由自动化转投递到 Slack 或 webhook">
  ...
  <rect x="250" y="80" width="160" height="60" rx="10" fill="var(--purple-soft)" stroke="var(--accent)" stroke-width="2"/><text x="330" y="102" text-anchor="middle" font-size="9" font-weight="700" fill="var(--accent-ink)">emit 决策</text><text x="330" y="119" text-anchor="middle" font-size="6.8" fill="var(--accent-ink)">applyStateMachine</text><text x="330" y="132" text-anchor="middle" font-size="6.5" fill="var(--muted)">PAUSED 直接跳过</text>
  <rect x="470" y="60" width="220" height="42" rx="10" fill="var(--bg)" stroke="var(--accent)" stroke-width="1.6"/><text x="580" y="79" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--accent-ink)">WebhookQueue（统一通知中枢）</text><text x="580" y="94" text-anchor="middle" font-size="6.5" fill="var(--muted)">monitor 只管发布，不管怎么投</text>
