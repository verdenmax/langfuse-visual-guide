...
<p>概念上，你在应用里写的埋点代码大致长这样（不同语言 SDK 形态略有差异，这里只示意「调用 → 事件」的对应关系；图中 <code>langfuse.trace()/.generation()</code> 是显式写法，现代 Python SDK 多用 <code>@observe</code> 装饰器，底层产生同样的 CREATE/UPDATE 事件）：</p>
...
event" mapping; the <code>langfuse.trace()/.generation()</code> calls shown are an explicit illustration — modern Python SDKs use the <code>@observe</code> decorator, which emits the same CREATE/UPDATE events):</p>
