...
<svg viewBox="0 0 720 230" role="img" aria-label="webhook SSRF 纵深防御：一个用户填的 URL 要依次通过协议白名单(仅http/https)、端口白名单(仅80/443)、主机名黑名单(localhost/metadata)、IP CIDR 黑名单(内网/loopback/link-local)、解析失败即拦截，全过才放行请求，任一不过即拒绝">
  ...
  <rect x="20" y="44" width="120" height="150" rx="9" fill="var(--bg)" stroke="var(--accent)"/><text x="80" y="64" text-anchor="middle" font-size="8" font-weight="700" fill="var(--accent-ink)">用户填的 URL</text><text x="80" y="84" text-anchor="middle" font-size="6.5" fill="var(--muted)">合法：</text><text x="80" y="96" text-anchor="middle" font-size="5.8" fill="var(--teal)">https://hooks.x.com/…</text><text x="80" y="120" text-anchor="middle" font-size="6.5" fill="var(--muted)">恶意：</text><text x="80" y="132" text-anchor="middle" font-size="5.6" fill="var(--red)">http://169.254.169.254</text><text x="80" y="144" text-anchor="middle" font-size="5.6" fill="var(--red)">http://localhost:6379</text><text x="80" y="156" text-anchor="middle" font-size="5.6" fill="var(--red)">file:///etc/passwd</text>
